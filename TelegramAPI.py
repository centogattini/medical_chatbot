...
class TelegramBot:
	...
	def start(self):
		...
		def print_times(bot, message, db):
			...
			#last_message = get_user_data(message.from_user.id, 'last_message')
			# if last_message:
			#     bot.edit_message_text( message_id = last_message,text = 'Выберите время для записи',
			#                         chat_id=message.chat.id, reply_markup=keyboard)
			# else:
			#     last_message = bot.send_message(message.from_user.id,'Выберите время для записи', reply_markup=keyboard)
			#     set_user_data(message.from_user.id, 'last_message',last_message.id)
			bot.send_message(message.from_user.id,'Выберите время для записи', reply_markup=keyboard)
		...
		def ask_2(message):
			...
			if tag == 'error':
				...
				self.bot.send_message(message.from_user.id, 'Кажется мы не нашли врача,который вам нужен. Мы можем записать вас к терапевту', reply_markup=keyboard)
				self.bot.register_next_step_handler(message, ask_3)

			else:
				print_dates(self.bot, message,db)
		...
		def ask_dates(message):
			if message.text == f'следующие {N_days} дат':
				...
				print_times(self.bot, message,db)
		...
		def bye_successful(message):
			...
			btn1 = types.KeyboardButton(f'/start')
			keyboard.add(btn1)
			self.bot.send_message(message.from_user.id,f'Спасибо за обращение! Вы записаны к врачу {get_user_data(message.from_user.id,"picked_doc")}, число: {get_user_data(message.from_user.id,"picked_date")}, время: {get_user_data(message.from_user.id,"picked_time")}',reply_markup = keyboard)
